...
def get_tile_properties(tmxdata, x, y, world_offset):
    ...
    try:
        ...
        properties = {"climbable": 0, "ground":0, "health":-10000, "points": 0, "provides":"", "requires":"", "solid":0}
    if properties is None:
        properties = {"climbable": 0, "ground":0, "health":0, "points": 0, "provides":"", "requires":"", "solid":0}
    return properties
...
def main5():   # Lesson 2... multiple animations, maintaining direction
    ...
    while not quit:
        ...
        if x < 140: 
            x = 140
            world_offset[0] += 10
        if x >= window.get_width()-140-25: 
            x = window.get_width()-140-25
            world_offset[0] -= 10
